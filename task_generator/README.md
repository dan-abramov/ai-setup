...
2. Отдельный статус генерации (`EMPTY/LOADING/SUCCESS/ERROR`) на странице не отображается.
3. Отправляет `skill` и `topic`.
4. Система выполняет flow: `submit -> generation -> validate -> create Task`.
5. При `SUCCESS` фронтенд сразу делает redirect на `/task/:id`.
6. На `/task/:id` показываются `task_description` и многострочное поле `solution_code` для ввода Ruby-кода решения.
7. Пользователь явно сохраняет `solution_code` кнопкой на странице задачи.
8. При `ERROR` пользователь получает код ошибки. Для `E204-E209` и `E301` доступно действие `Повторить`.
9. После закрытия вкладки задачу можно открыть повторно по URL `/task/:id`; поле `solution_code` остается предзаполненным сохраненным значением.
...
- При `200` также отображается editable многострочное поле `solution_code` с последним сохраненным значением для этой задачи.
...
## Контракт `PATCH /task/:id`

- HTTP `302`: сохранение прошло успешно, redirect обратно на `/task/:id`.
- Параметры: `task[solution_code]` (`text`, может быть пустым для очистки значения).
- Для несуществующей задачи возвращается `E302` (`404`).
- Для задачи с `description.blank?` возвращается `E303` (`422`).
- Проверка/исполнение Ruby-кода не выполняется.
...
Если `OPENROUTER_TIMEOUT_SECONDS` не задан, используется значение `15` секунд.
