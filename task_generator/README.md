...
7. При `ERROR` пользователь получает код ошибки. Для `E204-E209` и `E301` доступно действие `Повторить`.
8. После закрытия вкладки задачу можно открыть повторно по URL `/task/:id`.
...
- При `200` также отображается editable многострочное поле `solution_code` (UI-only, без проверки и сохранения в БД).
